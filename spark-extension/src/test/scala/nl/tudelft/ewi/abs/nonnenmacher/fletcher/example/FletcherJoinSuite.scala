...
class FletcherJoinSuite extends FunSuite with SparkSessionGenerator {
  ...
  override def withExtensions: Seq[SparkSessionExtensions => Unit] = Seq(ArrowParquetReaderExtension, FletcherJoinExampleExtension)

  //ignore("join on equality and sum up int column") {
    ...
    assert(sqlDF.queryExecution.executedPlan.find(_.isInstanceOf[FletcherJoinExampleExec]).isDefined)
